...
class nnUNetTrainer(object):
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # whole_cell_tp = tp[0] + tp[1]  
        # whole_cell_fp = fp[0] + fp[1]
        # whole_cell_fn = fn[0] + fn[1]
        # whole_cell_dc = 2 * whole_cell_tp / (2 * whole_cell_tp + whole_cell_fp + whole_cell_fn) if (whole_cell_tp + whole_cell_fp + whole_cell_fn) > 0 else np.nan

        # nucleus_dc = 2 * tp[1] / (2 * tp[1] + fp[1] + fn[1]) if (tp[1] + fp[1] + fn[1]) > 0 else np.nan

        # # average of whole cell and nucleus Dice scores
        # avg_dice_score = np.nanmean([whole_cell_dc, nucleus_dc])

        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in zip(tp, fp, fn)]]
        # self.logger.log('average_dice_score', avg_dice_score, self.current_epoch)
        # self.logger.log('whole_cell_dice', whole_cell_dc, self.current_epoch)
        # self.logger.log('nucleus_dice', nucleus_dc, self.current_epoch)        
